module.exports = function (fastify, opts, done) {
    ...
    fastify.route({
        ...
        handler: async (request, reply) => {
            ...
            const items = await r.table('comments').filter(where).orderBy(r.desc('id')).skip((p - 1) * pNum).limit(pNum).run()
            reply.send({ code: 0, items, total: await r.table('comments').filter(where).count().run() })
        }
    })
    done()
}